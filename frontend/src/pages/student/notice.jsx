...
const StudentNotifications = () => {
  ...
  const fetchEvents = async () => {
    setLoading(true);
    try {
      const studentClassId = userData?.student?.class?._id;
      if (!studentClassId) {
        setEvents([]);
        return;
      }

      const response = await authAxios.get(`/events/class/${studentClassId}`);
      const allEvents = response.data.success ? response.data.data : [];
  ...
  const getNotificationIcon = (type) => {
    switch (type) {
      ...
      case 'announcement': return <Bookmark className="h-5 w-5 text-purple-500" />;
                    ...
                    <div className="px-6 py-4 border-t border-gray-100 bg-gray-50">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <p className="text-gray-700">Start: {formatDate(event.startDate)}</p>
                          <p className="text-gray-700">End: {formatDate(event.endDate)}</p>
                        </div>
                        {event.location && (
                          <div>
                        ...
                        <div className="mt-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {event.classes.map((cls) => (
                              <span key={cls._id} className="px-2 py-1 bg-gray-100 text-gray-600 rounded-full text-xs">
                                {cls.name}
                              </span>
                            ))}
                          </div>
